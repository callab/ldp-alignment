...
# ~ 15min
present <- map(1:30, function(row) {
  data_frame(present = grepl(paste0("^",liwc[row,"word"],'$'), tokens$word,ignore.case = TRUE))}) %>%
  bind_cols()
names(present) <- pull(liwc,word)[1:30]

tokens_slice <- tokens[150000:155000,]
...
alignment <- long_present %>%
  ...
  group_by(person, category, measure) %>%
  ...
  select(person, category, alignment) %>%
