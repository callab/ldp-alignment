...
long_present<-read_feather("data/long_present.feather")
...
```{r NO LONGER ACCURATE: Get LIWC counts}
# split_utts <- split_utts %>% 
#   mutate(length = str_count(chat, ' '))
# 
# long_present <- long_present %>% 
#   unique(by=c("subject", "session", "person", "category", "run")) %>% 
...
#   na.omit() %>% 
#   select(-BA,-notBA,-BnotA, -notBnotA, -chat) %>% 
...
#          lag_conj = lag(conj), lag_conj = lag(discrep), lag_excl = lag(excl),
...
#   group_by(subject, session, person, lag_article, lag_certain, lag_conj,
#            lag_excl,
#            lag_i,
#            lag_incl,
#            lag_ipron,
...
# 
# 
# write_feather(together_present,'together_present.feather')
```

```{r Adjust LIWC counts for multi LIWC words}
### still isn't technically right, actually need to run for all utterances 
## see chunking strategy below
### may need to adjust here to account for utterances with '+' in them
split_utts <- split_utts %>% 
  mutate(length = str_count(chat, ' '))

long_present <- long_present %>% 
  select(-BA,-notBA,-BnotA, -notBnotA) %>% 
...
single_liwcs <- together_present %>% 
  filter(null >= 0) %>% 
  select (-chat)

# Get utterances containing multi-LIWC words
multi_liwcs <- together_present %>% 
  filter(null<0) %>%
  left_join(tokens, by=c('subject','session','person','run')) %>% 
  select(subject, session, person, run, word)
  
# Detect presence/absence of every LIWC word in every word in utterance
multi_present <- map(1:683, function(row) {
  data_frame(present = grepl(paste0("^",liwc[row,"word"],'$'),
                             multi_liwcs$word,ignore.case = TRUE))}) %>%
  bind_cols()
names(multi_present) <- pull(liwc,word)[1:683]

# Gather in interpretable way and index into point look-up table to establish counts
points_going <- multi_present %>% 
  bind_cols(multi_liwcs) %>% 
  gather(liwc_word, count, names(multi_present)) %>% 
  left_join(liwc_points, by = c("liwc_word" = "word")) %>%
  group_by(subject, session, person, category,run) %>%
  filter(count == TRUE) %>%
  summarise(count = sum(points)) %>%
  arrange(run) %>%
  group_by(subject,session, person, run, category) 

# Spread out, get length, and compute null
spread_points_going <- points_going %>% 
  spread(category, count, fill = 0) %>% 
  left_join(split_utts, by=c('subject','session','run','person')) %>% 
  mutate(null = length - (article + certain + conj + discrep + excl + i + incl + 
                            ipron + negate + preps + quant + tentat + we + you )) %>% 
...
# Still some negative nulls :(
spread_points_going %>% 
  filter(null<0) %>% 
  View()

spread_points_going <- spread_points_going %>% 
  mutate(null = case_when(null > 0 ~ null,
                          TRUE ~ 0)) %>% 
  ungroup()
  
# binds all utts together
fixed_together_present <- single_liwcs %>% 
  ungroup() %>% 
  bind_rows(spread_points_going) %>% 
  arrange(run) 
