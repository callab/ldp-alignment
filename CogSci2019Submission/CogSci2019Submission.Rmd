...
liwc <- read_tsv("../data/liwc2007_converted.tsv", col_names = FALSE) %>%
...
Children make vast linguistic strides within their first few years of life.
...
In light of this, some researchers have offered the linguistic tuning hypothesis, arguing that parents bolster their child's early language learning by calibrating the complexity of their speech to the particular abilities and needs of their children [@Montag:2015iy; @Snow:2018wf; @Thiessen:2005tx]. The idea is intuitive, but it is unclear at what level of language tuning occurs [@Hayes:1988ue; @Sokolov:1993cr; @Spivey:2006fa] and how overt it is [@Brown:1970wd; @Chouinard:2003kq; @HirshPasek:1984bd].
...
Following @Anonymous:r2JoRscQ, successive utterances from a speaker within a transcript were concatenated into a single utterance. Individual utterances were then transformed into binary vectors with indices indicating the presence or absence of each of the 14 LIWC categories. This pre-processing turned every transcript into a speaker-reply format: each utterance within a transcript was both a reply to the preceding utterance and a message to the next one. 
  ...
  Sampling was performed using Stan, a probabilistic programming language that implements Hamiltonian Monte Carlo sampling methods [@Carpenter:2017ke]. Posterior distributions for each parameter in the model were estimated using 500 iterations of Bayesian sampling, generating mean assessments with appropriate confidence intervals. \footnote{Data and code available at \url{https://github.com/callab/ldp-alignment}.} 
...
Alignment estimates ($\eta^{align}$) for parents and children were both estimated above zero, corroborating the findings of @Anonymous:r2JoRscQ in showing that both groups exhibit alignment (Figure 1). We also replicate the finding that parents appear to align more to their children than children align to their parents. 
  ...
  The model estimates changes in baseline category production across development ($\beta$) at approximately zero for parents, but significantly above zero for children, replicating previous findings. Alignment is estimated as having a significantly negative age effect ($\alpha$) for parents in this dataset, replicating an earlier finding that alignment from parents to children tends to decrease over their child's development (Figure 3). 

```{r parameters_plot, fig.width=3.25, fig.height=2.25, set.cap.width=T, num.cols.cap=1, cache = T, fig.cap = "Posterior parameter estimates for alignment ($\\eta$), developmental change in alignment ($\\alpha$), and developmental change in baseline function word production ($\\beta$) for both parents and children, as well estimated alignment between parents for a baseline. Bars indicate means, error-bars indicate 95\\% highest posterior density intervals generated via Bayesian sampling."}
...
```{r, eval=F}
demotable <- plot_parameters %>%
  arrange(parameter) %>%
  mutate(StandardError = (ci_upper-ci_lower)/2,
         Estimate = mean) %>%
  select(person, parameter,Estimate, StandardError) %>%
  xtable::xtable(., digits=2,caption = 'Parameter Estimates for Alignment with standard errors.')

print(demotable, type="latex", comment = F, table.placement = "H", include.rownames = FALSE, sanitize.text.function = function(x){x})
```
...
```{r coefficients xtable, results = 'asis', eval = T}
...
ppvt_parameters %>% 
  ...
  mutate(Estimate = mean,
         StandardError = (ci_upper-ci_lower)/2) %>% 
  select(parameter,Estimate, StandardError) %>% 
  rename(Parameter = parameter) %>% 
  slice(match(order, Parameter)) %>%
  xtable::xtable(., digits=2,caption = 'Parameter Estimates for PPVT predictors (and intercept) with standard errors. Parameters with "x" denote estimates of variable interaction.') %>% 
  print(., type="latex", comment = F, table.placement = "H", include.rownames=FALSE, sanitize.text.function = function(x){x})

# demotable <- ppvt_parameters %>%
#   mutate(parameter = replace(parameter, parameter == 'ppvt_education', "ppvt_mother_ed")) %>%
#   arrange(parameter) %>%
#   mutate(StandardError = (ci_upper-ci_lower)/2,
#          Estimate = mean) %>%
...
```{r demopar_plot, fig.width=3.5, fig.height=2.25, set.cap.width=T, num.cols.cap=1, cache = T, fig.cap = "Posterior parameter estimates for demographic effects on PPVT scores. Points indicate means, error-bars indicate 95\\% highest posterior density intervals.", eval = F}
plot_ppvt_parameters <- ppvt_parameters %>%
  mutate(parameter = gsub("ppvt_", "", parameter),
         ...
         person = if_else(str_detect(parameter, "ed"), "parent", person)) %>%
  mutate(parameter = case_when(parameter == "age_years" ~ "age",
                               str_detect(parameter, "align") ~ "alignment",
                               parameter == "mother_ed_slope" ~ "educ.",
                               parameter == "education" ~ "educ.",
...
ggplot(plot_ppvt_parameters, aes(x = parameter, y = mean, ymin = ci_lower, ymax = ci_upper,
  ...
  theme(axis.title.x = element_blank(),
        ...
        axis.text.x = element_text(size=5.25))
...
However, much of this prior work investigates tuning as an overt effort on behalf of parents or tuning with respect to content words, with less examining the potential role of low-level syntactic influence [@Hoff:2003kx]. @Anonymous:r2JoRscQ presents just such an examination, demonstrating using Bayesian hierarchical modeling that parents align to their children according to their particular language usage at the level of function word categories. This paper extends their model by applying it to a new socioeconomically diverse sample of families [@GoldinMeadow:2014hr] and leveraging the model's alignment estimates to predict language development outcomes.  
  
The analysis presented here replicates the findings of @Anonymous:r2JoRscQ, showing strong alignment effects for both parents and their children, a substantial age effect for baseline useage in children, and a significant negative effect of age on alignment for parents. Moreover, we demonstrate that these alignment estimates have substantial power in predicting vocabulary development measures, even in the presence of demographic features such as gender and socioeconomic status. We corroborate previous findings that female children tend to have higher PPVT scores that improve more gradually over time [@Lange:2016gv; @Kaushanskaya:2013gi]. We conflict with other findings that positively associate child PPVT scores with mother's education [@DiCesare:2013ip; @Schady:2011bb]; this may be due to idiosyncracies of our dataset, including its limited size. 

```{r hpds, fig.env = "figure*", fig.width=7, fig.height=2.75, fig.align = "center", set.cap.width=T, num.cols.cap=2, fig.cap = "Model-estimated changes in linguistic alignment over development. Points indicate the mean of the posterior distribution; shaded regions indicate 68\\% highest probability density intervals, equivalent to one standard deviation, for visualization purposes.", cache = T}
