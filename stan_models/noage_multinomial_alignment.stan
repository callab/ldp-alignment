...
parameters {
  ...
  real eta_ab_Marker [NumMarkers];         //lin. pred. for each marker+group alignment
    ...
    // maybe have alignment estimation for null cat but stipulate lag counts are always 0 to eliminate impact?
        ...
        for (Marker in 1:NumMarkers) {
          ...
          mu[Observation, Marker] = inv_logit(eta_observation[Observation, Marker] +
            (eta_ab_observation[Observation,Marker] * log(LagCounts[Observation, Marker]+1))); // no age effect on anything
...
model {
  ...
  eta_ab_Marker~ normal(eta_ab_pop, StdDev);

  //marker-dyad level distributions
  for(Observation in 1:NumObservations) {
    for (Marker in 1:NumMarkers){
      eta_observation[Observation, Marker] ~ normal(eta_pop_Marker[Marker], StdDev);
      eta_ab_observation[Observation, Marker] ~ normal(eta_ab_Marker[Marker], StdDev);
    }

    eta_observation[Observation, NumMarkers+1] ~ normal(eta_pop_Marker[NumMarkers+1], StdDev); //for estimating baseline for null
  }
