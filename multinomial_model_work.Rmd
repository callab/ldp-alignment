...
```{r Load Data}
together_present <- read_feather('data/together_present.feather')
```
...
fit <- stan('stanfits/noage_multinomial_alignment.stan', chains =1)
```

```{r Fit Age Multi Stan Model}
fit <- stan('stanfits/multinomial_alignment.stan', chains = 1)
```
