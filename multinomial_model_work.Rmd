...
```{r Load Data}
together_present <- read_feather('together_present.feather')

together_present <- read_csv('together_present.csv')

together_df <- read_csv('together_df.csv')
```

```{r Optional Filtering}
...
##### For full LIWC categories model
together_present <- together_present %>%
  filter(session ==5,
         subject == 22,
         person == "Child")
...
fit <- stan('noage_multinomial_alignment.stan', chains =1)
```

```{r Fit Age Multi Stan Model}
fit <- stan('multinomial_alignment.stan', chains = 1)
```
