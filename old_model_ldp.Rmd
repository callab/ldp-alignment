...
momed_mean_data <-  momed_data %>%
  ...
  mutate(parameter = factor(parameter, 
                            levels = c("eta_ab_subpop", "alpha_subpop"),
                            labels = c("alignment", "change")))

jpeg("momed_params.jpeg", width = 8, height = 4, units = "in", res = 600)
ggplot(momed_mean_data, aes(x = mother_ed, 
                        ymin = ci_lower, ymax = ci_upper, 
                        color = parameter,
                       y = empirical_stat, group = parameter, 
                       label = parameter)) + 
  geom_pointrange() + 
  geom_line()+
  geom_hline(yintercept=0,linetype='dashed') +
  scale_color_brewer(palette = "Set1") +
  geom_dl(method = list("last.qp", dl.trans(x = x + 0.2), cex = 1.2)) +
  labs(x = "Mother's Education", y = "Parameter Estimate") +
...
```{r}
plot_vals <- developmental_subpop_parameters  %>%
  left_join(measures %>% 
              filter(measure == "ppvt") %>%
              select(subject, measure, value, mother_ed)) %>%
  filter(person == "Parent", 
         parameter %in% c("alpha_subpop", "eta_ab_subpop")) %>%
  group_by(mother_ed, subject, parameter, mean) %>%
  summarise(mean_value = mean(value), 
            range_value = last(value) - first(value)) %>%
  filter(parameter %in% c("eta_ab_subpop", "alpha_subpop")) %>%
  spread(parameter, mean) 
  
jpeg("alignment_ppvt_group.jpeg", width = 4, height = 4, units = "in", res = 600)
ggplot(plot_vals, aes(x = eta_ab_subpop, y = range_value, 
color = mother_ed, group = 1)) + 
geom_point() + 
geom_smooth(method = "lm") + 
labs(x = "Alignment", y = "Gain in PPVT") +
theme_classic(base_size = 18)
dev.off()  
  
  %>%
  lm(range_value ~ eta_ab_subpop, data = .) %>%
  summary()
