...
# 
# subjs_over_10 <- raw_long_present %>%
...
# long_present_demos %>%
#   ungroup() %>%
#   select(subject, mother_education) %>%
#   distinct() %>%
...
divergent <- get_sampler_params(fit_hierarchical, inc_warmup=FALSE)[[1]][,'divergent__']

# fit <- stan("stan_models/alignmentcompletelyhierarchical.stan",
#             chains = 1,
#             iter = 350)
# saveRDS(fit, "demohier350iters.rds")
# 
...
demo_abs <- names(fit_hierarchical) %>% 
  as_data_frame() %>% 
  filter(str_detect(value, "mom") | str_detect(value, "gender")) %>% 
  pull()
...
parameter_cis %>% 
  ...
  geom_errorbar(aes(ymin=se_lower, ymax=se_upper), width = .2, position=position_dodge())+
  guides(fill=FALSE)+
...
long_present %>%
  group_by(age_years) %>%
  summarise(ppvt = mean(ppvt)) %>%
  ggplot(aes(x = age_years, y = ppvt)) +
  geom_point() + 
  geom_smooth()
```
