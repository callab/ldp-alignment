...
data {
  ...
  real factor_intercepts[NumObservations];
  real factor_slopes[NumObservations];
...
parameters {
  ...
  real<lower=0> factor_int_sigma; //error term for factor intercept linear regression
  real factor_int_intercept; //intercept term for factor intercept linear regression

  real<lower=0> factor_slope_sigma; //error term for factor slope linear regression
  real factor_slope_intercept; //intercept term for factor slope linear regression
  ...
  real factor_int_education; 
  real factor_int_female; 
  real factor_int_child_align;
  real factor_int_parent_align;

  real factor_slope_education; 
  real factor_slope_female; 
  real factor_slope_child_align;
  real factor_slope_parent_align;
...
model {
  ...
  // predict factor scores intercept and slope using demos AND parent and child alignment estimate
  for (Observation in 1:NumObservations){
    factor_intercepts[Observation] ~ normal(factor_int_intercept + 
      (mother_education[Observation] * factor_int_education)+ 
      (female[Observation] * factor_int_female) +
      (eta_ab_speaker[childid[Observation]] * factor_int_child_align) + 
      (eta_ab_speaker[parentid[Observation]] * factor_int_parent_align) 
      , factor_int_sigma);

    factor_slopes[Observation] ~ normal(factor_slope_intercept + 
      (mother_education[Observation] * factor_slope_education)+ 
      (female[Observation] * factor_slope_female) +
      (eta_ab_speaker[childid[Observation]] * factor_slope_child_align) + 
      (eta_ab_speaker[parentid[Observation]] * factor_slope_parent_align) 
      , factor_slope_sigma);
  }
