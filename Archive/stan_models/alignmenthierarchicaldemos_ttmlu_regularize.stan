...
data {
  ...
  real ppvt_vals[NumObservations];
  // real ppvt_slopes[NumObservations];
  real age_years[NumObservations];
  // vector<lower=0>[NumObservations] income_category;
  real mother_education[NumObservations];
  int<lower=0> female[NumObservations];
  real types[NumObservations]; 
  real tokens[NumObservations];
  real mlu[NumObservations]; 
...
parameters {
  ...
  real ppvt_types_slope;
  real ppvt_tokens_slope;
  real ppvt_mlu_slope;
...
model {
  ...
  // includes types count, tokens count, and mlu as controls

  // regularize ppvt predictors to 0
  ppvt_age_years ~ normal(0, StdDev);
  ppvt_education ~ normal(0, StdDev);
  ppvt_tokens ~ normal(0, StdDev);
  ppvt_types ~ normal(0, StdDev);
  ppvt_mlu ~ normal(0, StdDev);
  ppvt_mother_ed_slope ~ normal(0, StdDev);
  ppvt_types_slope ~ normal(0, StdDev);
  ppvt_tokens_slope ~ normal(0, StdDev);
  ppvt_mlu_slope ~ normal(0, StdDev);
  ...
  for (Observation in 1:NumObservations){
    ppvt_vals[Observation] ~ normal(ppvt_intercept + 
      ...
      (types[Observation] * ppvt_types) + 
      (tokens[Observation] * ppvt_tokens) + 
      (mlu[Observation] * ppvt_mlu) + 
      ...
      (types[Observation] * age_years[Observation]* ppvt_types_slope) + 
      (tokens[Observation] * age_years[Observation]* ppvt_tokens_slope) + 
      (mlu[Observation] * age_years[Observation]* ppvt_mlu_slope)
      , sigma);
  }
