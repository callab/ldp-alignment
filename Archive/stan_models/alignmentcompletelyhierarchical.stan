...
parameters {
  ...
  vector [NumSubPops] eta_ab_gender[NumSex]; // estimates for each subpop for each sex
  matrix [NumSex, NumSubPops] eta_ab_mothered[NumMomEd]; // estimates for each subpop for each sex for each momed

  vector [NumSubPops] alpha_gender[NumSex]; // estimates for each subpop for each sex
  matrix [NumSex, NumSubPops] alpha_mothered[NumMomEd]; // estimates for each subpop for each sex for each momed

  vector [NumSubPops] beta_gender[NumSex]; // estimates for each subpop for each sex
  matrix [NumSex, NumSubPops] beta_mothered[NumMomEd]; // estimates for each subpop for each sex for each momed
...
model {
  ...
  for(Gender in 1:NumSex){
    eta_ab_gender[Gender] ~ normal(eta_ab_subpop, StdDev);
    alpha_gender[Gender] ~ normal(alpha_subpop, StdDev);
    beta_gender[Gender] ~ normal(beta_subpop, StdDev);

  }

  // is this level of depth interpretable?
  for(MotherEd in 1:NumMomEd){
    for(Gender in 1:NumSex){
      eta_ab_mothered[MotherEd,Gender] ~ normal(eta_ab_gender[Gender], StdDev);
      alpha_mothered[MotherEd, Gender] ~ normal(alpha_gender[Gender], StdDev);
      beta_mothered[MotherEd, Gender] ~ normal(beta_gender[Gender], StdDev);
    }
  }
  ...
  for(Speaker in 1:NumSpeakers) {
    ...
    eta_ab_speaker[Speaker] ~ normal((eta_ab_mothered[SpeakerMomEd[Speaker],SpeakerSex[Speaker], SpeakerSubPop[Speaker]]), StdDev);

    alpha_speaker[Speaker] ~ normal((alpha_mothered[SpeakerMomEd[Speaker],SpeakerSex[Speaker], SpeakerSubPop[Speaker]]), StdDev);
    
    beta_speaker[Speaker] ~ normal((beta_mothered[SpeakerMomEd[Speaker],SpeakerSex[Speaker], SpeakerSubPop[Speaker]]), StdDev);
