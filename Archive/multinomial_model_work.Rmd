...
# model <- measures %>%
#   filter(measure == "ppvt") %>%
#   #filter(!measure %in% c("wppsi_block", "sent_comp")) %>%
#   group_by(measure) %>%
#   mutate(value = scale(value)) %>%
#   lmer(value ~ age_years + 
#          (age_years | subject), 
#        control = lmerControl(optCtrl = list( maxfun=20000),
#                              optimizer = "bobyqa"), data = .)
# 
model_setup <- measures %>%
  filter(!measure %in% c("wppsi_block", "sent_comp")) %>%
  group_by(measure) %>%
  mutate(value = scale(value)) %>%
  ungroup() %>%
...
contrasts(model_setup$measure) <- contr.sum(3)

model <- lmer(value ~ measure * age_years +
         (age_years + measure | subject),
       control = lmerControl(optCtrl = list( maxfun=20000),
                             optimizer = "bobyqa"), data = model_setup)

subj_effects <- ranef(model)$subject %>% 
  rownames_to_column("subj") %>%
  data.frame() %>%
  as_tibble() %>%
  rename(intercept = `X.Intercept.`, slope = age_years) %>%
  select(subj, intercept, slope)

write_csv(subj_effects, "subjs_effects_sum.csv")
```

```{r}
fit <- readRDS("CogSci2019Submission/data/hierppvt1000iters.rds")
