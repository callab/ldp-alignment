...
# Compute proportion for each category within utterance 
prop_together_present <- together_present %>% 
  arrange(run) %>% 
  select(subject, session, person, run, article, certain, conj, discrep, excl, i, incl, ipron, negate, preps,
         quant, tentat, we, you, length, null) %>%
  ungroup() %>% 
  mutate_at(vars("article", "certain", 'conj', "discrep", "excl", "i", "incl", "ipron", "negate", "preps", "quant", "tentat",
                 "we","you", "null"),
            function(x) x/.$length) %>% 
...
write_feather(prop_together_present, 'data/prop_together_present.feather')

## SANITY CHECK - No props over 1!
# prop_together_present %>% 
#   select(-subject, -session, -person, -run, -length) %>% 
#   filter_all(any_vars(.>1))


## OLD VERSION - COMPUTES LAG COUNTS AND GROUPS BY UTTERANCE
